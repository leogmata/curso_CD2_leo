---
title: "Modelagem de Temperatura Máxima Plausível no Brasil"
subtitle: "Ciência de Dados Aplicados à Epidemiologia II"
...
Este documento apresenta uma proposta educacional para construção, interpolação e visualização de uma superfície de temperatura máxima plausível para o Brasil.\
\
O objetivo é demonstrar, de forma aplicada, como dados ambientais podem ser simulados com base em critérios geográficos e climáticos e, posteriormente, utilizados em análises espaciais no R.
...
## Leitura da base territorial do Brasil

```{r}
br_proj  <- read_country(year = 2020) |> 
  st_transform(crs_proj)

ufs_proj <- read_state(year = 2020) |> 
  st_transform(crs_proj)

bb <- st_bbox(br_proj)
```

## Obtenção das estações meteorológicas
...
## Construção da temperatura máxima plausível

As temperaturas utilizadas neste projeto não representam valores observados reais. Elas são simuladas com base em critérios de plausibilidade climática, considerando latitude, altitude, proximidade do litoral e domínio climático predominante.
...
## Transformação das estações para projeção métrica
...
## Conversão da interpolação para raster
...
## Recorte do raster para o território brasileiro
...
## Mapa da temperatura máxima plausível interpolada
...
Este documento apresentou um fluxo completo de construção, interpolação e visualização de temperaturas máximas plausíveis para o Brasil.

O projeto tem finalidade educacional e não deve ser interpretado como uma representação de temperaturas reais observadas. Sua principal contribuição é demonstrar como critérios geográficos e ambientais podem ser combinados para gerar dados simulados, permitindo o aprendizado de técnicas de análise espacial, interpolação e visualização no contexto da epidemiologia.
